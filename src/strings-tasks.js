...
function removeFirstOccurrences(str, value) {
  ...
  const firstSubStr = str.slice(0, subStrIndex);
  const secondSubStr = str.slice(subStrIndex + value.length);
...
function removeLastOccurrences(str, value) {
  ...
  if (str.length - 1 === subStrIndex + value.length - 1) {
    return str.slice(0, subStrIndex);
  }
  const firstSubStr = str.slice(0, subStrIndex);
  const secondSubStr = str.slice(subStrIndex + value.length);
  return firstSubStr + secondSubStr;
...
function countVowels(str) {
  ...
  let acc = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(str[i])) acc += 1;
  }
  return acc;
}
...
function isPalindrome(str) {
  ...
  if (str.toLowerCase().replace(regexReplacer, '') === reverseStr) return true;
  return false;
}
...
function findLongestWord(sentence) {
  const strLikeArr = sentence.split(' ');
  let indexOfLongestWord = 0;
  let curElLength = 0;
  strLikeArr.forEach((el, index) => {
    if (curElLength < el.length) {
      indexOfLongestWord = index;
      curElLength = el.length;
    }
  });
  return strLikeArr[indexOfLongestWord];
}
...
function reverseWords(str) {
  const strLikeArr = str.split(' ');
  const reverseWordInStr = [];
  strLikeArr.forEach((el) => {
    reverseWordInStr.push(el.split('').reverse().join(''));
  });
  return reverseWordInStr.join(' ');
}
...
function invertCase(str) {
  const strLikeArr = str.split('');
  const resStr = strLikeArr.map((el) => {
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[a-z]/gi,
    (letter) => encodedDictionary[dictionary.indexOf(letter)]
  );
}
